...
#XD
class TimeDistributedMaxoutDense(MaskedLayer):
    input_ndim = 3

    def __init__(self, output_dim, nb_feature=4, init='glorot_uniform', activation='linear', weights=None,
                 W_regularizer=None, b_regularizer=None, activity_regularizer=None,
                 W_constraint=None, b_constraint=None, input_dim=None, input_length=None, **kwargs):
        self.output_dim = output_dim
        ...
        if self.input_dim:
            kwargs['input_shape'] = (self.input_length, self.input_dim)
        super(TimeDistributedMaxoutDense, self).__init__(**kwargs)
    ...
    def get_output(self, train=False):
        X = self.get_input(train)
        output = self.activation(T.max(T.dot(X.dimshuffle(1, 0, 2), self.W) + self.b, axis=2))
        return output.dimshuffle(1, 0, 2)
    ...
    def get_config(self):
        config = {"name": self.__class__.__name__,
                  ...
                  "input_dim": self.input_dim,
                  "input_length": self.input_length}
        base_config = super(TimeDistributedMaxoutDense, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
