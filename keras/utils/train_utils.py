...
from keras.layers.recurrent_xd import ReducedLSTM, LSTM, SimpleRNN
...
def build_reduced_lstm(input_dim, h0_dim=40, h1_dim=None, output_dim=1, layer_type=TimeDistributedDense, optimizer='rmsprop', name='rlstm'):
    model = Sequential()  
    ...
    if h1_dim is not None:
        model.add(layer_type(h1_dim, 
                    ...
                    W_regularizer=l2(0.0005),
                    activation='relu'))
    model.add(ReducedLSTM(output_dim, return_sequences=True))
...
def load_rlstm(name='rlstm'):
    rlstm = model_from_yaml(open('rlstm1h' + '.yaml').read())
