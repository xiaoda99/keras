...
#load RNN model
RNN_model_path='forXiaoDa20151124.pkl.gz'
...
def RNNpredict(pm25, gfs, date_time, pm25_mean, pred_range, downsample=1):
#    print 'pm25'
#    print pm25[0]
#    print 'gfs'
#    print gfs[0]
...
#    gfs[:,2]=np.sqrt(gfs[:,2]**2+gfs[:,3]**2)
    ...
    #predict
    batch=pm25.shape[0]
    cnt=np.repeat(np.eye(steps,dtype='float32').reshape(1,steps,steps),batch,axis=0)
    a=RNNobj.pred_fun(gfs,pm25in,cnt)
#    print 'a =', a
        
    #output scaling back
    output=a*100+pm25_mean[:,2:42,0]
    return output
...
pred1 = predict_all_batch(data[data.shape[0]*3./4:], RNNpredict)  
